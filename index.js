...
(async () => {
  const originRawRes = await r2('https://registry.npmjs.org/bootstrap')
    .response;
  const originRes = await originRawRes.json();
  const rawOriginReleases = originRes.versions;
  const originReleasesTagName = Object.keys(rawOriginReleases).map(
    (release) => `v${release}`,
  );
  ...
  const nojsReleasesTagName = Object.keys(rawNojsReleases).map(
    (release) => `v${release}`,
  );
  ...
  const checkout = (repo, versions, tagLength) => {
    ...
    let commit = null;
    Git.Reference.dwim(repo, `refs/tags/${version}`)
      ...
      .then(async () => {
        ...
        npminit.load({}, (er, npm) => {
          if (er) throw er;
          npm.config.setCredentialsByURI(registry, {
          ...
          npm.commands.publish(['.'], async () => {
            if (currentPosition < tagLength - 1) {
              ...
              Object.keys(distTags).map((tag) => {
                npm.commands['dist-tag']([
                  'add',
                  `bootstrap-nojs@${distTags[tag]}`,
                  tag,
                ]);
                return false;
              });
  ...
  mainTagLength = needRelease.length;
  ...
  checkout(repo, needRelease, mainTagLength);
  await trash(['bootstrap*', '!bootstrap']);
})();
